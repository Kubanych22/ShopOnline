...
export const scss = () => gulp
  ...
  // .pipe(gulpif(!dev, gulp.dest(path.dist.css))) // удалить, если не нужен index.css, а только index.min.css
...
if (!dev) {
  webpackConf1.module.rules.push({
    test: /\.(js)$/,
    exclude: [/node_modules/, /path.src.blog/, /path.src.goods/],
    loader: 'babel-loader',
  })
}
...
if (!dev) {
  webpackConf2.module.rules.push({
    test: /\.(js)$/,
    exclude: [/node_modules/, /path.src.index/],
    loader: 'babel-loader',
  })
}
...
if (!dev) {
  webpackConf2.module.rules.push({
    test: /\.(js)$/,
    exclude: [/node_modules/, /path.src.index/, /path.src.blog/],
    loader: 'babel-loader',
  })
}
...
export const js = () => gulp
  ...
  // Если включить, то в режиме production файлы js также копируется в папку dist
  // Т.е. в этой папке окажутся и минифицированный и не минифицированный файлы
...
export const base = gulp.parallel(html, scss, js, jsBlog, jsGoods, img, svg, webp, avif, copy);
